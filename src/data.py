...
def _get_data(csv_path):
    ...
    if isinstance(csv_path, list):
        print('Loading data from {}'.format(csv_path.join(', ')))
...
def _to_bq(data, project_id, dataset_id, table_id):
    ...
    data.to_gbq(project_id=project_id, destination_table=_table_name, if_exists='replace',
                verbose=True, private_key=SERVICE_ACCOUNT_PATH)
